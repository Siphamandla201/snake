...
function animate() {
  ...
  if (keys.w.pressed) {
    // Calculate the ship's velocity based on the updated rotation angle
    const forwardSpeed = SPEED;
    const backwardSpeed = -SPEED;
    const shipSpeed = keys.w.pressed ? forwardSpeed : backwardSpeed;
    
    // Calculate both x and y components of the velocity based on the rotation
    player.velocity.x = Math.cos(player.rotation + 1) * shipSpeed;
    player.velocity.y = Math.sin(player.rotation + -2) * shipSpeed;
  } else {
  ...
  switch (event.code) {
    ...
    case 'Space':
      ...
      projectiles.push(
        new Projectile({
          position: {
            x: player.position.x + Math.cos(player.rotation + -1) * 10,
            y: player.position.y + Math.sin(player.rotation + -2) * 10,
          },
          velocity: {
