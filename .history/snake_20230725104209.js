...
function update() {
    ...
    if (snakeBody.length) {
        snakeBody[0] = {x : snake.x, y: snakeY} 
    }
    ...
    context.fillRect(snake.x,snakeY, blockSize, blockSize);
    ...
    if (snake.x < 0 || snake.x > col * blockSize || snakeY < 0 || snakeY > rows * blockSize) {
        gameOver = true;
        context.font("20px");
    }
    ...
    for(let i = 0; i < snakeBody.length; i++) {
        if (snake.x == snakeBody[i].x && snakeY == snakeBody[i].y) {
            gameOver = true;
            context.font("20px");
        }
    }
