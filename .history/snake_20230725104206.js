...
function update() {
    ...
    if (snakeBody.length) {
        snakeBody[0] = {x : snake., y: snakeY} 
    }


    context.fillStyle = 'lime';
    snake. += velocityX * blockSize;
    snakeY += velocityY * blockSize;
    context.fillRect(snake.,snakeY, blockSize, blockSize);
    ...
    if (snake. < 0 || snake. > col * blockSize || snakeY < 0 || snakeY > rows * blockSize) {
        gameOver = true;
        context.font("20px");
    }
    ...
    for(let i = 0; i < snakeBody.length; i++) {
        if (snake. == snakeBody[i].x && snakeY == snakeBody[i].y) {
            gameOver = true;
            context.font("20px");
        }
    }
