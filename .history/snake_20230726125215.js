...
function liveLeft() {
    if(gameOver) {
        ...
        // The game is not over, so reset the number of lives.
        gameLives = 2; // You may set the initial number of lives here or elsewhere in your code.
    }
}
Save to grepper
With this updated code, the game will decrement the gameLives variable by one whenever the gameOver condition is met. It will only reset the game and set the number of lives to 2 when the game is not over. This way, the game will properly handle multiple lives before completely resetting.
