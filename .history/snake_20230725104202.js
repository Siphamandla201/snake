...
function update() {
    ...
    context.fillRect(,snakeY, blockSize, blockSize);
    ...
    if ( < 0 ||  > col * blockSize || snakeY < 0 || snakeY > rows * blockSize) {
        gameOver = true;
        context.font("20px");
    }
    ...
    for(let i = 0; i < snakeBody.length; i++) {
        if ( == snakeBody[i].x && snakeY == snakeBody[i].y) {
            gameOver = true;
            context.font("20px");
        }
    }
