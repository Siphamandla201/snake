...
function animate() {
  ...
  if (keys.w.pressed) {
    ...
    const shipSpeed = keys.ArrowRight.pressed ? forwardSpeed : backwardSpeed;
    
    // Calculate both x and y components of the velocity based on the rotation
    player.velocity.x = Math.cos(player.rotation) * shipSpeed;
    player.velocity.y = Math.sin(player.rotation) * shipSpeed;
  } else {
