...
window.onload = function () {
    board.height = window.innerHeight / 3;
...
function update() {
    ...
    context.fillRect(food.x,food., blockSize, blockSize);

    // to check if food is eaten by snake 
    if(snake.x == food.x && snake.y == food.) {
        snakeBody.push({x: food.x, y: food.});
        score += 100; 
        placeFood();
    }
    ...
    if (snake.x < 0 || snake.x > col * blockSize || snake.y < 0 || snake.y > rows * blockSize) {
        gameOver = true;
        context.font("20px");
    }
...
function placeFood() {
    food.x = Math.floor(Math.random() * col) * blockSize;
    food. = Math.floor(Math.random() * col) * blockSize;
}
