...
function update() {
    ...
    if (snakeBody.length) {
        snakeBody[0] = {x : sn, y: snakeY} 
    }


    context.fillStyle = 'lime';
    sn += velocityX * blockSize;
    snakeY += velocityY * blockSize;
    context.fillRect(sn,snakeY, blockSize, blockSize);
    ...
    if (sn < 0 || sn > col * blockSize || snakeY < 0 || snakeY > rows * blockSize) {
        gameOver = true;
        context.font("20px");
    }
    ...
    for(let i = 0; i < snakeBody.length; i++) {
        if (sn == snakeBody[i].x && snakeY == snakeBody[i].y) {
            gameOver = true;
            context.font("20px");
        }
    }
