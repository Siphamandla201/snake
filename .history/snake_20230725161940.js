...
function checkAlive() {
    let snakeHead = { x: snake.x, y: snake.y };
    
    if (snakeHead.x < 0 ) {
        gameOver = true;
        snakeHead.x = snakeHead;
    } else if (snakeHead.x > board.width) {
        gameOver = true;
        snakeHead.x = board.width - (snakeHead * blockSize;
    }
    
    // Check for collision with top and bottom walls
    if (snakeHead.y < 0) {  // Check for collision with top 
        gameOver = true;
        snakeHead.y = snakeHead; // Keep the snake at the top edge of the board
    
    } else if (snakeHead.y + blockSize > board.height) {  // Check for collision bottom walls
        gameOver = true;
        snakeHead.y = board.height - snakeHead; // Keep the snake at the bottom edge of the board
