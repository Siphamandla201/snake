...
function update() {
    ...
    if (snakeBody.length) {
        snakeBody[0] = {x : snake.x, y: snake.} 
    }


    context.fillStyle = 'lime';
    snake.x += velocityX * blockSize;
    snake. += velocityY * blockSize;
    context.fillRect(snake.x,snake., blockSize, blockSize);
    ...
    if (snake.x < 0 || snake.x > col * blockSize || snake. < 0 || snake. > rows * blockSize) {
        gameOver = true;
        context.font("20px");
    }
    ...
    for(let i = 0; i < snakeBody.length; i++) {
        if (snake.x == snakeBody[i].x && snake. == snakeBody[i].y) {
            gameOver = true;
            context.font("20px");
        }
    }
